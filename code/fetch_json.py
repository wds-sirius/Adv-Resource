...
# get episode Temp json
files = os.listdir('./_temp')
fileslist = [x.replace('.json', '') for x in files if x.endswith('.json')]
    ...
    for data in GameStoryMasterlist["StoryMaster"]["Side"]:
        for group in data["Groups"]:
            ...
            for ep in EPData:
                ...
                # 生成故事文檔
                try:
                    if str(ep["EpisodeMasterId"]) in fileslist:
                        if os.path.exists(os.path.join('./_temp', f'{ep["EpisodeMasterId"]}.json')):
                            addedKeyData = json.load(open(os.path.join('./_temp', f'{ep["EpisodeMasterId"]}.json'), 'rb'))
                            ...
                            os.remove(os.path.join('./_temp', f'{ep["EpisodeMasterId"]}.json'))
                        # 檢查列表中是否存在
...
if masterlistres.status_code == 200:
    for data in masterlistres.json():
        # 生成故事文檔
        try:
            if str(data["EpisodeMasterId"]) in fileslist:
                if os.path.exists(os.path.join('./_temp', f'{data["EpisodeMasterId"]}.json')):
                    addedKeyData = json.load(open(os.path.join('./_temp', f'{data["EpisodeMasterId"]}.json'), 'rb'))
                    ...
                    os.remove(os.path.join('./_temp', f'{data["EpisodeMasterId"]}.json'))
                # 檢查列表中是否存在
